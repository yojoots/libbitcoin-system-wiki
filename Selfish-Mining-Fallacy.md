...
This statement assumes a concept of "fair share" that is foreign to Bitcoin, another straw man. A [miner](Glossary#miner) is [rewarded](Glossary#reward) based on their [blocks](Glossary#block) that reach [maturity](Glossary#maturity), not as a proportion of actual [hash rate](Glossary#hash-rate).
...
This is the source of the fallacy. It is not an "attack" for conventional wisdom to be incorrect, it is an error in the presumed conventional wisdom. Selfish mining implies that Bitcoin exhibits [pooling](Glossary#pooling) pressure, yet this is a [well-established flaw](Proximity-Premium-Flaw). To the extent that the optimization is effective, it presents a distinct pooling pressure from that caused by [latency](Glossary#latency), [variance](Glossary#variance), [variation](Glossary#variation), or [distortion](Glossary#distortion). All pooling pressures reduce the number of miners, exposing Bitcoin to attacks.
