...
It is possible that other similar networks may be deployed by independent operators. This would resolve the dependency on the single node. Nevertheless, success implies that a substantial number of people will become entirely dependent upon just a few nodes, all operating with [state](Glossary#state) licenses. These people are essentially accepting a persistent [Sybil-attack](https://en.wikipedia.org/wiki/Sybil_attack).

This is not comparable to having a similarly small number of Internet providers, since thousands of independently operated nodes are potentially reachable through any number of protocols. As such the most compelling *free services* scenario may be North Korea, where Internet is generally unavailable and the risk of a dependency on a few licensed satellite providers is a reasonable trade-off against oppressive currency controls.
